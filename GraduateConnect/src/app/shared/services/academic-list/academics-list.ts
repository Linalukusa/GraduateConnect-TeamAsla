...
import { CrudService } from 'src/app/shared/crud.service';  // CRUD API service class
import { Academic } from 'src/app/shared/academic';   // Student interface class for Data types.
...
@Component({
  ...
  templateUrl: './academics-list.component.html',
  styleUrls: ['./academics-list.component.css']
})

export class AcademicsListComponent implements OnInit {
  ...
  deleteaCADEMIC(academic) {
    if (window.confirm('Are sure you want to delete this student ?')) { // Asking from user before Deleting student data.
      this.crudApi.DeleteStudent(academic.$key) // Using Delete student API to delete student.
